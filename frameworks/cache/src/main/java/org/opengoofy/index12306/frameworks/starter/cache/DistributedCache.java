...
public interface DistributedCache extends Cache{
    /**
     * 获取缓存，如果查询结果为空，调用CacheLoader加载缓存
     */
    ...
    /**
     * 以一种“安全”的方式获取缓存，如果查询结果为空，调用cacheLoader加载缓存
     * 通过此方式防止程序中可能出现的：缓存击穿，缓存雪崩场景，使用于不被外部直接待用的接口
     */
    ...
    /**
     * 以一种“安全”的方式获取缓存，如果查询结果为空，调用cacheLoader加载缓存
     * 通过此方式防止程序中可能出现的：缓存击穿，缓存雪崩场景，需要客户端传递布隆过滤器，使用于不被外部直接待用的接口
     */
