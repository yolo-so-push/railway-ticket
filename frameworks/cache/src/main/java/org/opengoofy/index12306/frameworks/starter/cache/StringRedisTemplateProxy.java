...
package org.opengoofy.index12306.frameworks.starter.cache;
...
import org.opengoofy.index12306.frameworks.starter.cache.config.RedisDistributedProperties;
...
public class StringRedisTemplateProxy implements DistributedCache{
    ...
    public <T> T get(String key, Class<T> clazz, CacheLoader<T> cacheLoader, long timeout, TimeUnit timeUnit) {
        ...
        if (!CacheUtil.isNullOfBlank(result)){
            return result;
        }
    ...
    public <T> T safeGet(String key, Class<T> clazz, CacheLoader<T> cacheLoader, long timeout, TimeUnit timeUnit, RBloomFilter<String> bloomFilter, CacheGetFilter<String> cacheCheckFilter, CacheGetIfAbsent<String> cacheGetIfAbsent) {
        T result = get(key, clazz);
        if (!CacheUtil.isNullOfBlank(result)
                || Optional.ofNullable(cacheCheckFilter)
                        .map(e-> e.filter(key))
                        .orElse(false)
                ||Optional.ofNullable(bloomFilter)
                .map(e->e.contains(key))
                .orElse(false)){
            RLock lock = redissonClient.getLock(SAFE_GET_DISTRIBUTED_LOCK_KEY_PREFIX + key);
            lock.lock();
            try {
                if (CacheUtil.isNullOfBlank(result=get(key,clazz))){
                    if (CacheUtil.isNullOfBlank(result=loadAndSet(key,cacheLoader,timeout,timeUnit,true,bloomFilter))){
                        Optional.ofNullable(cacheGetIfAbsent).ifPresent(e->e.execute(key));
                    }
                }
            }finally {
                lock.unlock();
