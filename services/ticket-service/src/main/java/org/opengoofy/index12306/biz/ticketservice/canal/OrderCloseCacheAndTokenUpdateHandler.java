...
public class OrderCloseCacheAndTokenUpdateHandler implements AbstractExecuteStrategy<CanalBinlogEvent, Void> {
    ...
    public void execute(CanalBinlogEvent message) {
        ...
        List<Map<String, Object>> messageDataList = message.getData().stream()
                ...
                .filter(e -> Objects.equals(e.get("status"), "30"))
                .toList();
        if (CollUtil.isEmpty(messageDataList)){
            return;
        }
        for (Map<String, Object> each : messageDataList) {
            Result<TicketOrderDetailRespDTO> detailRespDTOResult = ticketOrderRemoteService.queryTicketOrderByOrderSn(each.get("orderSn").toString());
            TicketOrderDetailRespDTO orderDetailRespDTO = detailRespDTOResult.getData();
            if (detailRespDTOResult.isSuccess()&&orderDetailRespDTO!=null){
                String trainId = orderDetailRespDTO.getTrainId().toString();
                List<TicketOrderPassengerDetailRespDTO> passengerDetails = orderDetailRespDTO.getPassengerDetails();
                seatService.unlock(trainId, orderDetailRespDTO.getDeparture(), orderDetailRespDTO.getArrival(), BeanUtil.convert(passengerDetails, TrainPurchaseTicketRespDTO.class));
                ticketAvailabilityTokenBucket.rollbackInBucket(orderDetailRespDTO);
            }
        }
    }
