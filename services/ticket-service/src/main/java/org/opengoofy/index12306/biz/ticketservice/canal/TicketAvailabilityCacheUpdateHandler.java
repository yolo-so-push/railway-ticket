...
import static org.opengoofy.index12306.biz.ticketservice.common.constant.Index12306Constant.SEAT_STATUS;
    ...
    public void execute(CanalBinlogEvent message) {
        ...
        for (int i = 0; i < message.getData().size(); i++) {
            ...
            if (oldDataMap.get(SEAT_STATUS)!=null&& StrUtil.isNotBlank(oldDataMap.get(SEAT_STATUS).toString())){
                Map<String, Object> currentMap = message.getData().get(i);
                if (StrUtil.equalsAny(currentMap.get(SEAT_STATUS).toString(),String.valueOf(SeatStatusEnum.AVAILABLE.getCode()),String.valueOf(SeatStatusEnum.LOCKED.getCode()))){
                    actualOldDataList.add(oldDataMap);
                    messageDataList.add(currentMap);
                }
            }
        }
        //座位状态数据没有发生变化
        ...
        for (int i = 0; i < messageDataList.size(); i++) {
            Map<String, Object> currentData = messageDataList.get(i);
            ...
            String trainId = currentData.get("train_id").toString();
            String hashCacheKey=TRAIN_STATION_REMAINING_TICKET+trainId+"_"+currentData.get("start_station")+"_"+currentData.get("end_station");
            Map<Integer, Integer> seatTypeMap = cacheChangeKeyMap.get(hashCacheKey);
            if (CollUtil.isEmpty(seatTypeMap)){
                seatTypeMap=new HashMap<>();
            }
            Integer seatType = Integer.parseInt(currentData.get("seat_type").toString());
